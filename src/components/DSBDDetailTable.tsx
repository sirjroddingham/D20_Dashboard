...
interface DSBDetailTableProps {
  rows: DSBRow[];
}

export default function DSBDetailTable({ rows }: DSBDetailTableProps) {
  const [sortConfig, setSortConfig] = useState<SortConfig>({ key: 'concessionDate', direction: 'desc' });
        ...
        COLUMNS.some(col => {
          if (col.key === 'defectCategories') {
            return row.defectCategories.some(c =>
              DSB_DEFECT_LABELS[c as keyof typeof DSB_DEFECT_LABELS]?.toLowerCase().includes(term)
            );
          }
          return String(row[col.key as keyof DSBRow] || '').toLowerCase().includes(term);
        })
      );
    }
  ...
  const handleExportCSV = useCallback(() => {
    ...
    link.href = url;
    link.download = `dsb-details-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
    ...
    >
      <div className="border-b border-surface-3 px-5 py-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-text-subtle">DSB Defect Details</h3>
